...
public class ServerSend
{
    ...
    public static void RayShot(Vector3 _shotOrigin, Vector3 _rayHitPoint)
    {
        using (Packet _packet = new Packet((int)ServerPackets.rayShot))
        {
            _packet.Write(_rayHitPoint);
            _packet.Write(_shotOrigin);

            SendTCPDataToAll(_packet);
        }
    }
    public static void Pong(int _toClient, float _pong)
    {
